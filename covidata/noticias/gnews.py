...
def executar_busca():
    ...
    # Salva resultados
    if not path.exists(config.diretorio_noticias):
        os.makedirs(config.diretorio_noticias)

    caminho_arquivo_resultante = os.path.join(config.diretorio_noticias, f'noticias_n_{len(df)}.xlsx')
