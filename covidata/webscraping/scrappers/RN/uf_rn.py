...
class PortalTransparencia_RN(SeleniumDownloader):
    ...
    def __init__(self):
        super().__init__(path.join(str(config.diretorio_dados)[:-18], 'dados', 'RN', 'portal_transparencia', 'RioGrandeNorte'),
                         config.url_pt_RN)
    ...
    def _executar(self):
        ...
        list_files = glob(path.join(str(config.diretorio_dados)[:-18], 'dados', 'RN', 'portal_transparencia', 'RioGrandeNorte/*'))
        ...
        df_contratos = pd.read_csv(path.join(str(config.diretorio_dados)[:-18], 'dados', 'RN', 'portal_transparencia', 'RioGrandeNorte', file_name),
                                   sep=';', error_bad_lines=False)

        # Cria arquivo "xlsx" e aloca file handler de escrita para a variável "writer"
        with pd.ExcelWriter(path.join(str(config.diretorio_dados)[:-18], 'dados', 'RN', 'portal_transparencia',
        ...
        os.unlink(path.join(str(config.diretorio_dados)[:-18], 'dados', 'RN', 'portal_transparencia', 'RioGrandeNorte', file_name))
